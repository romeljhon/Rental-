...
const mockGlobalItems: RentalItem[] = [
  ...
  { id: '3', name: 'Vintage Leather Jacket', description: 'Stylish vintage leather jacket, medium size.', category: 'Apparel', pricePerDay: 20, imageUrl: 'https://placehold.co/600x400.png', availabilityStatus: 'Rented', owner: mockGlobalUsers[0], location: 'Los Angeles, CA', rating: 4.2, reviewsCount: 8 },
  { id: 'j1', name: 'Portable Projector HD', description: 'Great for movie nights. Bright and clear picture.', category: 'Electronics', pricePerDay: 40, imageUrl: 'https://placehold.co/600x400.png', availabilityStatus: 'Available', owner: mockGlobalUsers[1], location: 'Chicago, IL', rating: 4.6, reviewsCount: 12, features: ['1080p Resolution', 'Built-in Speaker'] },
  { id: 'j2', name: 'Camping Tent for 4 People', description: 'Spacious and waterproof, ideal for family camping trips.', category: 'Sports & Outdoors', pricePerDay: 25, imageUrl: 'https://placehold.co/600x400.png', availabilityStatus: 'Available', owner: mockGlobalUsers[1], location: 'Chicago, IL', rating: 4.3, reviewsCount: 7, features: ['Sleeps 4', 'Waterproof', 'Easy Setup'] },
  { id: 's1', name: 'Electric Guitar & Amp Set', description: 'Beginner friendly electric guitar with a small practice amplifier.', category: 'Musical Instruments', pricePerDay: 30, imageUrl: 'https://placehold.co/600x400.png', availabilityStatus: 'Available', owner: mockGlobalUsers[2], location: 'Austin, TX', rating: 4.0, reviewsCount: 5, features: ['Includes Cable', 'Practice Amp'] },
  { id: 's2', name: 'Professional 4K Drone', description: 'Capture stunning aerial footage with this 4K drone. Long flight time.', category: 'Electronics', pricePerDay: 70, imageUrl: 'https://placehold.co/600x400.png', availabilityStatus: 'Rented', owner: mockGlobalUsers[2], location: 'Austin, TX', rating: 4.9, reviewsCount: 10 },
  { id: 'j3', name: 'Paddleboard (SUP)', description: 'Inflatable stand-up paddleboard, comes with pump and paddle.', category: 'Sports & Outdoors', pricePerDay: 30, imageUrl: 'https://placehold.co/600x400.png', availabilityStatus: 'Available', owner: mockGlobalUsers[1], location: 'Miami, FL', rating: 4.7, reviewsCount: 9, features: ['Inflatable', 'Carry Bag', 'Pump included'] },
];
...
export default async function UserProfilePage({ params }: { params: { userId: string } }) {
  ...
  if (!userData) {
    // Or a more styled "User not found" page
    return (
        ...
        <CardHeader className="pb-4">
          <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
            ...
            <div>
              ...
              <p className="text-muted-foreground">Member since {new Date().getFullYear() - Math.floor(Math.random() * 3 + 1)}</p> {/* Mock join year */}
      ...
      <section>
        ...
        {items.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {items.map((item) => (
              <ItemCard key={item.id} item={item} /> // No onEdit or onRemove for public view
            ))}
          </div>
        ) : (
...
// Optional: Generate static paths if you have a known set of users and want to pre-render
// export async function generateStaticParams() {
//   return mockGlobalUsers.map(user => ({
//     userId: user.id,
//   }));
// }
